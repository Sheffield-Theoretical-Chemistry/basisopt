...
class EvenTemperedToBasisStrategy(Strategy):
        ...
        if not self.initialised[element]:
            ...
            try:
                ...
                for shell in basis[element.lower()]:
                    if len(shell.exps) < 2:
                        self._INITIAL_Guess.append([shell.exps])
                    else:
                        self._INITIAL_Guess.append([_INITIAL_GUESS])
    ...
    def set_active(self, values: np.ndarray, basis: InternalBasis, element: str):
        ...
        if n < self.n_exp_cutoff:
            self.shells[self._step] = values

        else:
            self.shells[self._step] = (c, x, _)
            self.set_basis_shell(basis, element)
