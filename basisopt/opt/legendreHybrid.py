...
from basisopt.basis.guesses import bse_guess, legendre_guess, null_guess, load_guess
...
class LegendrePairsHybrid(Strategy):
    ...
    def set_basis_shell(self, basis: InternalBasis, element: str):
        ...
        if self.shells[self._step][0][1] < self.n_exp_cutoff:
            basis[element][self._step].exps = self.pre.inverse(abs(self.shells[self._step][0][0]), **self.pre.params)
        else:
    ...
    def initialise(self, basis: InternalBasis, element: str):
        ...
        if not self.initialised[element]:
            try:
                ...
                for shell in basis[element.lower()]:
                    if len(shell.exps) < self.n_exp_cutoff:
                        ...
                        self._INITIAL_Guess.append([(np.array([1.0 if i % 2 == 0 else -1. for i in range(2)]),len(shell.exps))])
            ...
            self.shells = [[(shell.exps, len(shell.exps))] if len(shell.exps) < self.n_exp_cutoff else [shell.leg_params] for shell in basis[element]]
