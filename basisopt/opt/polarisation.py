...
from basisopt.util import bo_logger, dict_decode, get_composition
...
class PolarizationStrategyConverge(Strategy):
...
class PolarizationStrategy(Strategy):
    ...
    def next(self, basis: InternalBasis, element: str, objective: float) -> bool:
        ...
        if self.first_run:
            ...
            self.all_combinations['config'].append(get_composition({element: basis[element]}, element))
        ...
        try:
            ...
            if all(self._testing):
                for test in self._combinations:
                    self.all_combinations['config'].append(get_composition({element: test[1]}, element))
